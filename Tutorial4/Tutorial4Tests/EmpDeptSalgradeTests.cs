...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        var result = emps.Join(grades, e => 1, s => 1, (e, s) => new {e, s}).Where(es => es.e.Sal > es.s.Losal && es.e.Sal < es.s.Hisal).Select(es => new {es.e.EName, es.s.Grade}).ToList();
    ...
    public void ShouldReturnEmployeesEarningMoreThanDeptAverage()
    {
        ...
        var result = emps.Where(e => e.Sal > emps.Where(f => f.DeptNo == e.DeptNo).Average(f => f.Sal)).Select(e => e.EName).ToList(); 
